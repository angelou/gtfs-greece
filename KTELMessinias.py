...
url = "http://www.ktelmessinias.gr/?module=default&pages_id=44&lang=el"
driver.get(url)
...
schedule.AddAgency(agency_id = "KTELMessinias",
                   name = "Suburban Transportations of Messinia Perfecture",
                   timezone = "Europe/Athens",
                   url = "http://www.ktelmessinias.gr")


# Retrieve route links
route_links = driver.find_elements_by_xpath("//div[@id='tree_menu']/ul/li/a")
...
# Skip the first URL, it lists the ticker prices
for url in route_urls[1:]:
    print("Fetching %s" % url)
    driver.get(url)

    # Retrieve all route matrices
    routes = driver.find_elements_by_xpath("//div[@class='content']/h3")

    # Do a bit of cleanup in routes found, because some other elements apart from route titles are in h3
    h3s = routes
    for h3_index, h3 in enumerate(h3s):
        if "-" not in h3.text.strip().encode("utf-8"):
            del routes[h3_index]
    ...
    for route_index, route in enumerate(routes):
        ...
        # Sometimes, there are empty h3 inside the HTML
        if not route_id:
            continue
        ...
        # Retrieve trips
        trips = driver.find_elements_by_xpath("//div[@class='content']/table[%s]/tbody/tr" % (route_index + 1))
        ...
        for trip in trips[1:]:
            ...
            # Sometimes, the last line would be just empty, without tds
            if not tds:
                continue
            ...
            departure_time = re.search("\d{2}:\d{2}", tds[0].text.strip().encode("utf-8").replace(".", ":").replace(";", ":")).group()
